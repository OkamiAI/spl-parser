...
class TmLanguageGenerator:
    ...
    def __parse_template(self):
        ...
        self.commands_block = self.template["repository"][
            f"commands.{COMMAND_PLACEHOLDER}"
        ]
        self.arguments_block = self.template["repository"][
            f"arguments.{COMMAND_PLACEHOLDER}"
        ]
        self.functions_block = self.template["repository"][
            f"functions.{COMMAND_PLACEHOLDER}"
        ]
        self.operators_block = self.template["repository"][
            f"operators.{COMMAND_PLACEHOLDER}"
        ]
        ...
        self.grammar["repository"]["template_expressions"] = {
            "patterns": [
                {
        ...
        self.grammar["patterns"].append({"include": "#template_expressions"})
    ...
    def add_command(self, spl_command):
        ...
        commands_block = self.generate_grammar_block(
            spl_command.name,
            self.commands_block,
            source=ALIAS_PLACEHOLDER,
            targets=command_aliases,
        )
        self.grammar["repository"]["commands"]["patterns"].append(
            self.generate_include(spl_command.name, "commands")
        )
        self.grammar["repository"][f"commands.{spl_command.name}"] = commands_block

        if spl_command.arguments:
            arguments_block = self.generate_grammar_block(
                spl_command.name,
                self.arguments_block,
                source=ARGUMENT_PLACEHOLDER,
                targets=spl_command.arguments,
            )
            self.grammar["repository"][f"commands.{spl_command.name}"][
                "patterns"
            ].append(self.generate_include(spl_command.name, "arguments"))
            self.grammar["repository"][f"arguments.{spl_command.name}"] = (
                arguments_block
            )

        if spl_command.functions:
            functions_block = self.generate_grammar_block(
                spl_command.name,
                self.functions_block,
                source=FUNCTION_PLACEHOLDER,
                targets=spl_command.functions,
            )
            self.grammar["repository"][f"commands.{spl_command.name}"][
                "patterns"
            ].append(self.generate_include(spl_command.name, "functions"))
            self.grammar["repository"][f"functions.{spl_command.name}"] = (
                functions_block
            )

        if spl_command.operators:
            operators_block = self.generate_grammar_block(
                spl_command.name,
                self.operators_block,
                source=OPERATOR_PLACEHOLDER,
                targets=spl_command.operators,
            )
            self.grammar["repository"][f"commands.{spl_command.name}"][
                "patterns"
            ].append(self.generate_include(spl_command.name, "operators"))
            self.grammar["repository"][f"operators.{spl_command.name}"] = (
                operators_block
            )
